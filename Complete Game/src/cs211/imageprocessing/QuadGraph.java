...
public class QuadGraph {
	...
	private List<int[]> cycles = new ArrayList<int[]>();// the list of all
														// cycles of length 4
	...
	public QuadGraph(List<PVector> lines, int width, int height,
		...
		int n = lines.size();
		graph = new int[n * (n + 1) / 2][2];// The maximum possible number of
											// edges is sum(0..n) = n * (n +
											// 1)/2
		int idx = 0;
	...
	/**
	 * This methods selects the best quad available following the three
	 * criterion of the pdf
	 * 
	 * @return the 4 corners of a good quad (coordinates in pixel), if nothing
	 *         good was found the last good quad
	 */
	public PVector[] selectBestQuad() {
		...
		while (it.hasNext()) {
			PVector[] n = it.next();
			if (!isConvex(n[0], n[1], n[2], n[3])
					|| !validArea(n[0], n[1], n[2], n[3], lastGoodArea * 1.2f,
							lastGoodArea * 0.8f)
		...
		if (candidates.size() == 0) {
			...
			Iterator<PVector[]> itx = candidates.iterator();
			while (itx.hasNext()) {
				PVector[] n = itx.next();
				if (!validArea(n[0], n[1], n[2], n[3], lastGoodArea * 1.3f,
						lastGoodArea * 0.7f)) {
					itx.remove();
				}
			}
		...
		if (candidates.size() != 0) {
			...
			lastGoodArea = area(lastGoodQuad[0], lastGoodQuad[1],
					lastGoodQuad[2], lastGoodQuad[3]);
		}
	 ...
	 * 
	 * @param corners
	 *            an array of the 4 corners of the quad
	 */
	...
	public void drawQuad(int[] quadLines) {
		...
		dummyContext.fill(dummyContext.color(
		...
		dummyContext.quad(c12.x, c12.y, c23.x, c23.y, c34.x, c34.y, c41.x,
				c41.y);
	}
				...
				if (graph[i][y] == n) {// edge refers to our current node
					...
					if (!visited(x, path)) {// neighbor node not on path yet
						...
						findNewCycles(sub);
							...
							&& (x == path[path.length - 1])) {// cycle found
	...
	// compare path against known cyclesreturn true, iff path is not a known
	// cycle
	 ...
	 * @param c1
	 */
	private static boolean isConvex(PVector c1, PVector c2, PVector c3,
			PVector c4) {
	 ...
	 */
	private static boolean validArea(PVector c1, PVector c2, PVector c3,
	...
	/**
	 * Computes the (cosine) of the four angles of the quad, and check they are
	 * all large enough (the quad representing our board should be close to a
	 * rectangle)
	 */
	private static boolean nonFlatQuad(PVector c1, PVector c2, PVector c3,
			PVector c4) {
